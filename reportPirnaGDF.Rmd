...
mutData <- rbindlist(pirnaGDF["adjRegion:piRNA", "mutData"], idcol = "mutID")
...
meltMUTdata <- melt.data.table(
  data = mutData[ , c(1, 2, 6, 10, 12, 14, 16, 18)], id.vars = "mutID"
)
meltMUTdata[ , `:=`(
...
tableINDEL <- meltMUTdata[ , table(
  variable[AF.Tipo == "AF > 0" & `Mutação.Tipo` == "INDEL"])]
tableSNP   <- meltMUTdata[ , table(
  variable[AF.Tipo == "AF > 0" & `Mutação.Tipo` == "SNP"])]
